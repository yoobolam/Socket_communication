...
public class TCPserver {
	public static void main(String[] args) throws IOException { // 메인함수, IOException
		...
		serverSocket = new ServerSocket(8885); // 서버소켓 생성
		...
		try {
			...
			out = new PrintWriter(clientSocket.getOutputStream(), true); // String 타입을 stream 형태로 변환하여
																			// 전송한다는 뜻.
			in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream())); // 소켓에서 넘오는 stream 형태의 문자를 얻은
																							// 후 읽어 들어서 //bufferstream
																							// 형태로 in 에 저장.
